...
SETTLING_TIME  = 1.5
OUTPUT_DIR     = 'missions'
DRONE_DELAY    = 60.0   # seconds between consecutive drone takeoffs
...
def plan_transit_altitudes(paths, cells, rlat, rlon):
    n = len(paths)
    launch = np.array([0., 0.])
    # Build transit LineStrings and travel times to first waypoint
    lines = []
    transit_times = []  # time drone i takes to fly from launch to its first WP
    for i in range(n):
        if paths[i]:
            sx, sy = ll2xy(paths[i][0][0], paths[i][0][1], rlat, rlon)
            seg = LineString([launch, (sx, sy)])
            lines.append(seg)
            dist = math.hypot(sx, sy)
            transit_times.append(dist / CRUISE_SPEED)
        else:
            lines.append(None)
            transit_times.append(0.)
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            # Drone j takes off DRONE_DELAY*(j-i) seconds after drone i
            delay = DRONE_DELAY * (j - i)
            # Time window when drone i is in transit: [0, transit_times[i]]
            # Time window when drone j is in transit: [delay, delay + transit_times[j]]
            i_start, i_end = 0., transit_times[i]
            j_start, j_end = delay, delay + transit_times[j]
            windows_overlap = i_start < j_end and j_start < i_end
            if lines[i].intersects(lines[j]) and windows_overlap:
                if i not in conflicts[j]: conflicts[j].append(i)
                if j not in conflicts[i]: conflicts[i].append(j)
        # Also check: drone i transit vs drone j's cell (original spatial check)
        for j in range(n):
            if i == j or cells[j].is_empty: continue
            if lines[i].intersects(cells[j]):
                ix = lines[i].intersection(cells[j])
                if ix.length > 2.0 and j not in conflicts[i]:
...
def plot_mission(cells, paths, infos, times, fence, rlat, rlon, alt_plans, save_path):
    ...
    hdr = f'{"Drone":>6}{"Area":>8}{"WPs":>6}{"Travel":>8}{"Cover":>8}{"Delay":>7}{"Total":>8}{"Dist":>8}\n{"─"*61}\n'
    body = ''.join(f'  D{i+1:<4}{infos[i].get("area",0):>7.0f}{infos[i].get("n_wp",0):>6}'
                   ...
                   f'{i*DRONE_DELAY:>6.0f}s{times[i]:>7.1f}s{infos[i].get("dist",0):>7.0f}m\n' for i in range(n))
    ...
    foot = (f'\n{"─"*61}\n  Max: {max(tots):.1f}s ({max(tots)/60:.1f}min)\n'
            ...
            f'  Delay: {DRONE_DELAY:.0f}s between drones\n'
...
def main():
    ...
    # Account for DRONE_DELAY: drone i takes off i*DRONE_DELAY seconds after drone 0
    effective_times = np.array([times[i] + i * DRONE_DELAY for i in range(N_DRONES)])

    print(f'\n  {"─"*55}')
    for i in range(N_DRONES):
        info, ap = infos[i], alt_plans[i]
        a = f'{ap["transit_alt"]:.0f}→{ap["coverage_alt"]:.0f}m' if ap['needs_climb'] else f'{ap["coverage_alt"]:.0f}m'
        delay_s = i * DRONE_DELAY
        print(f'  D{i+1}: {info["area"]:.0f}m² | {info["n_wp"]} wps | '
              f'travel={info["t_travel"]:.1f}s cover={info["t_coverage"]:.1f}s '
              f'delay={delay_s:.0f}s total={effective_times[i]:.1f}s | alt={a}')
    print(f'  {"─"*55}')
    tots = [t for t in effective_times if t > 0]
    if tots:
        print(f'  Range: {min(tots):.1f}–{max(tots):.1f}s '
              f'(Δ{max(tots)-min(tots):.1f}s / {(max(tots)-min(tots))/np.mean(tots)*100:.1f}%)')

    os.makedirs(OUTPUT_DIR, exist_ok=True)
    plot_mission(cells, paths, infos, effective_times, fence, rlat, rlon,
