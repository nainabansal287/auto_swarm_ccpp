...
mavproxy.py --retries 5 --master tcp:127.0.0.1:5760 --sitl 127.0.0.1:5501 --out 127.0.0.1:14550 --map --console"""
...
import math
import threading
import main_new #change if name of main_new file changes
import glob
import logging
...
MIN_HORIZ_DIST   = 5.0   # metres — trigger hold if closer than this
#MIN_ALTI_DIST = 2.0
AVOIDANCE_HOLD_S = 2.0   # seconds between re-checks while holding
LIFT             = 2.0  # alti increase in case of collision

CONNECT_TIMEOUT  = 600    # seconds — wait for first heartbeat
...
DRONES: List[Tuple[int, str, int]] = [
    ...
    (1,  'udpin:0.0.0.0.:14560',  1),
    (2,  'udpin:0.0.0.0:14570',  2),
    ]
...
def run_planner(launch_positions: dict):
    ...
    log.info("═" * 62)
    main_new.LAUNCH_LATLON   = launch_positions[1]
    main_new.LAUNCH_POSITIONS = launch_positions
    main_new.main()
...
def drone_thread(state: DroneState, all_states: List[DroneState],gps_barrier: threading.Barrier,
...
def main():
    ...
    for s in states:
        t = threading.Thread(
            target=drone_thread,
            args=(s, states, gps_barrier, skip_prearm),
            name=f"D{s.index}",
            daemon=True,
        )
        threads.append(t)
